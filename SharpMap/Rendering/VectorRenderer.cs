...
namespace SharpMap.Rendering
{
    ...
    public static class VectorRenderer
    {
        ...
        /// <param name="map">Map reference</param>
        [MethodImpl(MethodImplOptions.Synchronized)]
        public static void DrawPoint(Graphics g, Coordinate point, Brush b, float size, PointF offset, MapViewport map)
        {
        ...
        /// <param name="map">Map reference</param>
        [MethodImpl(MethodImplOptions.Synchronized)]
        public static RectangleF DrawPointEx(Graphics g, Coordinate point, Brush b, float size, PointF offset, MapViewport map)
        {
            if (point == null)
                return RectangleF.Empty;

            var pp = map.WorldToImage(point);
        ...
        /// <param name="map">Map reference</param>
        [MethodImpl(MethodImplOptions.Synchronized)]
        public static void DrawPoint(IPointSymbolizer symbolizer, Graphics g, NetTopologySuite.Geometries.Point point, MapViewport map) => DrawPointEx(symbolizer, g, point, map);
        ...
        /// <param name="map">Map reference</param>
        [MethodImpl(MethodImplOptions.Synchronized)]
        public static void DrawPoint(Graphics g, Coordinate point, Image symbol, float symbolscale, PointF offset,
        ...
        /// <param name="map">Map reference</param>
        [MethodImpl(MethodImplOptions.Synchronized)]
        public static RectangleF DrawPointEx(Graphics g, Coordinate point, Image symbol, float symbolScale, PointF offset,
            ...
            PointF offset, float rotation, MapViewport map)
        {
            ...
            foreach (NetTopologySuite.Geometries.Point pointGeometry in points)
            {
                var rect = DrawPointEx(g, pointGeometry.Coordinate, symbol, symbolScale, offset, rotation, map);
                canvasArea = rect.ExpandToInclude(canvasArea);
            }
            return canvasArea;
        }
        ...
        public static RectangleF DrawMultiPointEx(Graphics g, MultiPoint points, Brush brush, float size, PointF offset, MapViewport map)
        {
            ...
            foreach (NetTopologySuite.Geometries.Point pointGeometry in points)
            {
                var rect = DrawPointEx(g, pointGeometry.Coordinate, brush, size, offset, map);
                canvasArea = rect.ExpandToInclude(canvasArea);
            }
