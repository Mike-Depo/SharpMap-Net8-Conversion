    ...
    public class GeometryFeatureProviderTest : ProviderTest
    {
        ...
        public void GetItemByID()
        {
            ...
            for (var i = 0; i < 5; i++)
            {
                var row = fdt.NewRow();
                row[0] = (uint)i;
                row.Geometry = gf.CreatePoint(new Coordinate(i, i));
                fdt.AddRow(row);
            }
        ...
        public void TestFindGeoNearPoint()
        {
            ...
            var res = FindGeoNearPoint(gf.CreatePoint(new Coordinate(0.1, 0.1)), layer, 0.2d);
            NUnit.Framework.Assert.IsNotNull(res);
            NUnit.Framework.Assert.AreEqual(0, (uint)res[0]);
        }

public static SharpMap.Data.FeatureDataRow FindGeoNearPoint(
    Point point, SharpMap.Layers.VectorLayer layer, double amountGrow)
{
